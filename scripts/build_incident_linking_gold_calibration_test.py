...
with open(GOLD_PATH, 'r') as f:
        gold_clusters = json.load(f)
total_pairs = len(gold_clusters)
target_count = round(total_pairs * calibration_ratio)
target_positive_count = round(sum(record['same_event'] for record in gold_clusters) * calibration_ratio)
target_negative_count = round(sum(not record['same_event'] for record in gold_clusters) * calibration_ratio)
gold_categories = {record['category'] for record in gold_clusters}
target_category_counts = {}
for category in gold_categories:
    target_category_counts[category] = round(sum(record['category'] == category for record in gold_clusters) * calibration_ratio)
target_cross_category_count = round(sum(record['category_match'] is False for record in gold_clusters) * calibration_ratio)
...
def calculate_split_difference(records):
...
def split_gold_clusters():
    parent = {}
    for pair in gold_clusters:
    ...
    for root in roots:
        ...
        added_difference = calculate_split_difference(added_clusters)
        current_difference = calculate_split_difference(calibration_clusters)
    ...
    assert calibration_pairs | test_pairs == {pair_key(record) for record in gold_clusters}
    ...
    assert calibration_categories == gold_categories
    assert test_categories == gold_categories
    assert abs(len(calibration_clusters) / total_pairs - calibration_ratio) <= 0.05
    ...
    gold_distribution = calculate_distribution(gold_clusters)
    calibration_distribution = calculate_distribution(calibration_clusters)
    test_distribution = calculate_distribution(test_clusters)
    
    gold_positive_ratio = gold_distribution['positive_ratio']
    calibration_positive_ratio = calibration_distribution['positive_ratio']
    test_positive_ratio = test_distribution['positive_ratio']
    assert abs(calibration_positive_ratio - gold_positive_ratio) <= 0.10
    assert abs(test_positive_ratio - gold_positive_ratio) <= 0.10
    for category in gold_categories:
        gold_category_count = gold_distribution['category_counts'][category]
        calibration_category_count = calibration_distribution['category_counts'][category]
        assigned_ratio = (calibration_category_count / gold_category_count)
        assert abs(assigned_ratio - calibration_ratio) <= 0.15
    ...
    with open(CLUSTER_SPLIT_MANIFEST_PATH, 'w') as f:
        json.dump({
            ...
            'gold_pair_count': len(gold_clusters),
    ...
    print('Successfully split gold dataset of clusters.')
    print('Gold pairs:', len(gold_clusters))
